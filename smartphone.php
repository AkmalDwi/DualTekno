...
<head>
    ...
    <title>Smartphone - DualTekno</title>
...
<body>
    ...
    <header class="bg-primary py-3">
        <div class="container px-4 px-lg-5 my-5">
            <h1 class="text-center text-white">Smartphone</h1>
        </div>
    </header>
        ...
        <div class="container px-4 px-lg-5 mt-5">
            <form method="GET" class="mb-4">
                <select name="brand_filter" class="form-select" onchange="this.form.submit()">
                    ...
                    <?php
                    $stmt = $conn->prepare("SELECT DISTINCT brand FROM `products` WHERE category = 'Smartphone'");
                    $stmt->execute();
                    $result = $stmt->get_result();
    ...
    <section class="products">
        <div class="container px-4 px-lg-5 mt-5">
            <div class="row gx-4 gx-lg-5 row-cols-2 row-cols-md-3 row-cols-xl-4 justify-content-center">
                ...
                if ($brand_filter) {
                    $stmt = $conn->prepare("SELECT * FROM `products` WHERE category = 'Smartphone' AND brand = ? LIMIT 6");
                    $stmt->bind_param("s", $brand_filter);
                } else {
                    $stmt = $conn->prepare("SELECT * FROM `products` WHERE category = 'Smartphone' LIMIT 6");
                }
